function Compare(controlData) {
    ...
    obj.bindGrid = function() {
        ...
        for(var i=0; i<obj.p1Data.length; i++) {
            ...
            for(var j=0; j<obj.p2Data.length; j++) {
                if(obj.p1Data[i].leaderboardId == obj.p2Data[j].leaderboardId) {
                    ...
                    var mapInfo = mapList.find(x => x.uid === obj.p1Data[i].leaderboardId);
                    ...
                    resultData["songAuthorName"] = obj.p1Data[i].songAuthorName;
                    resultData["diff"] = obj.p1Data[i].diff;
                    if(obj.p1Data[i].rank < obj.p2Data[j].rank) {
                    ...
                    resultData["p1_rank"] = obj.p1Data[i].rank;
                    resultData["p2_rank"] = obj.p2Data[j].rank;
                    resultData["rank_gap"] = obj.p2Data[j].rank - obj.p1Data[i].rank;
                    p1RankGap += obj.p2Data[j].rank - obj.p1Data[i].rank;
                    ...
                    resultData["p1_pp"] = obj.p1Data[i].pp;
                    resultData["p2_pp"] = obj.p2Data[j].pp;
                    resultData["pp_gap"] = (obj.p1Data[i].pp - obj.p2Data[j].pp).toFixed(2);
                    p1PPGap += parseFloat(obj.p1Data[i].pp - obj.p2Data[j].pp);

                    var p1Date = new Date(obj.p1Data[i].timeSet).toISOString().
                        replace(/T/, ' ').      // replace T with a space
                        replace(/\..+/, '');
                    var p2Date = new Date(obj.p2Data[j].timeSet).toISOString().
                        replace(/T/, ' ').      // replace T with a space
                        replace(/\..+/, '');
            ...
            if (!isDuplicate) {
                ...
                var p1Date = new Date(obj.p1Data[i].timeSet).toISOString().
                    ...
                    replace(/\..+/, '');
                resultData["p1_timeSet"] = p1Date;
                // var p2Date = new Date(obj.p2Data[i].timeSet).toISOString().
                //     replace(/T/, ' ').      // replace T with a space
                //     replace(/\..+/, '');
        ...
        for(var i=0; i<obj.p2Data.length; i++) {
            ...
            if (!isDuplicate) {
                ...
                resultData["p2_rank"] = obj.p2Data[i].rank;
                resultData["rank_gap"] = 0;
                resultData["p1_accuracy"] = 0;
                resultData["p2_accuracy"] = obj.p2Data[i].accuracy;
                resultData["accuracy_gap"] = 0;
                resultData["p1_pp"] = 0;
                resultData["p2_pp"] = obj.p2Data[i].pp;
                resultData["pp_gap"] = 0;
    ...
    obj.compareApiCall = function (apiID) {
		...
		var callback_get_all_player1 = function(data) {
            obj.dataCount1 = (data.scores != undefined) ? data.scores.length : 0;
            if (obj.dataCount1 > 0) {
                for(var i=0; i<data.scores.length; i++) {
                    data.scores[i].accuracy = (data.scores[i].maxScore > 0) ? 
                        ((data.scores[i].score * 100) / data.scores[i].maxScore).toFixed(2) : 
                        0;
                        data.scores[i].mypp = (data.scores[i].pp * data.scores[i].weight).toFixed(2);
                        data.scores[i].diff = (data.scores[i].difficulty == 1) ? "Easy" : 
                        (data.scores[i].difficulty == 3) ? "Normal" : 
                        (data.scores[i].difficulty == 5) ? "Hard" : 
                        (data.scores[i].difficulty == 7) ? "Expert" : 
                        (data.scores[i].difficulty == 9) ? "Expert+" : 
                        data.scores[i].difficulty;
                    // #region 개발자 영역 > paging(more) 처리를 위한 셋팅
                    obj.p1Data.push(data.scores[i]);
                    if(i == data.scores.length - 1) {
                        obj.pageIndex1++;
                    }
                    // #endregion
                }
            ...
            if (obj.statusAllSearch1 == "ready" || obj.statusAllSearch1 == "process" || obj.statusAllSearch1 == "resume") {
                if (obj.pageIndex1 <= obj.max_page1) {
                    ...
                    if (obj.statusAllSearch1 == "complete" && obj.statusAllSearch2 == "complete") {
                        panelAlertLayer("All users data searched.", "success");
                        obj.bindGrid();
                    }
                    return null
                }
		...
		var callback_get_all_player2 = function(data) {
            obj.dataCount2 = (data.scores != undefined) ? data.scores.length : 0;
            if (obj.dataCount2 > 0) {
                for(var i=0; i<data.scores.length; i++) {
                    data.scores[i].accuracy = (data.scores[i].maxScore > 0) ? 
                        ((data.scores[i].score * 100) / data.scores[i].maxScore).toFixed(2) : 
